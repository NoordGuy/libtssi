...
//
// Crc table of polynomial 0x04c11db7
// see http://www.libpng.org/pub/png/spec/1.2/PNG-CRCAppendix.html 
// details: ETSI EN 300 468 Annex CRC decoder model

const TS_DWORD kMpeg2PsiCrcLookUpTable[] = {
...
TS_BOOL	tssi::Table::CheckCrc32(TS_PBYTE pbuf, unsigned in_len)
{
    TS_DWORD crc_accumulated = 0xffffffff;
    unsigned i, j;

    for (j = 0; j < in_len; ++j, ++pbuf) {
        i = (( crc_accumulated >> 24) ^ (*pbuf)) & 0xff;
        crc_accumulated = ((crc_accumulated << 8 ) ^ kMpeg2PsiCrcLookUpTable [i]);
    }
    return crc_accumulated == 0x00000000;
}
...
TS_BOOL	tssi::Table::Process(TS_PBYTE pbuf, unsigned in_len, TS_BOOL extended_table, TS_BOOL &new_version)
{
	if (in_len < 3) return TS_FALSE;
		
	m_table_id = PSI_TABLE_ID_VALUE(pbuf);
	TS_WORD table_id_ext = NIT_NETWORK_ID_VALUE(pbuf);
	m_section_length = PSI_SECTION_LENGTH_VALUE(pbuf);

	TS_BYTE version = PSI_VERSION_NUMBER_VALUE(pbuf);
	m_section_number = PSI_SECTION_NUMBER_VALUE(pbuf);
	m_last_section_number = PSI_LAST_SECTION_NUMBER_VALUE(pbuf);
	...
	TS_WORD id_value = 0;
	if (extended_table) {
		id_value = NTOH_S(WORD_VALUE(pbuf,8));
	}
	...
	if (!extended_table) {
		for (x = 0; x < m_section_list.size(); ++x)
			if ((m_section_list[x].table_id == m_table_id) && 
				(m_section_list[x].section_number == m_section_number) && 
				(m_section_list[x].table_id_ext == table_id_ext)) {
					if (m_section_list[x].current_version == version) {
						new_version = TS_FALSE;
						break;
					} else {
						m_section_list[x].current_version = version;
						new_version = TS_TRUE;
						break;
					}
		...
		for (x = 0; x < m_section_list.size(); ++x)
			if ((m_section_list[x].table_id == m_table_id) && 
				(m_section_list[x].section_number == m_section_number) && 
				(m_section_list[x].table_id_ext == table_id_ext) &&
				(m_section_list[x].identification_value == id_value)) {
					if (m_section_list[x].current_version == version) {
						new_version = TS_FALSE;
						break;
					} else {
	...
	// section not found? add
	if (x == m_section_list.size()) {
		SectionVersion section;
		section.table_id = m_table_id;
		section.section_number = m_section_number;
		section.table_id_ext = table_id_ext;
		section.identification_value = id_value;
...
TS_BOOL	tssi::Table::Process(TS_PBYTE pbuf, unsigned in_len)
{
	// minimal setup
	if (in_len < 1) return TS_FALSE;
	m_table_id = PSI_TABLE_ID_VALUE(pbuf);
...
TS_VOID tssi::Table::SetProcessCallback(TS_CALLBACK callback, TS_PVOID data)
{
	m_callback = callback;
